...
static bool Putchar(char c, int x, int y) {
    int cur_x = cursor_pos % (SCREEN_WIDTH - 2);
    int cur_y = (GetCursorLineWrapped() + cursor_pos / (SCREEN_WIDTH - 2)) - scroll;
...
int CommonSaveOpen(struct line* save_line) {
...
int OpenFile(void) {
    ...
    int res = CommonSaveOpen(save_line);
    if (res != 0) {
        return res;
    }
